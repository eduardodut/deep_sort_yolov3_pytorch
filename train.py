...
def train(
    ...
    init_seeds()
    weights = 'weights' + os.sep + os.path.basename(cfg).split('.')[0] + os.sep
    ...
    vis = Visualizer(env=os.path.basename(cfg).split('.')[0])

    # Initialize model
    model = Darknet(cfg, img_size).to(device)

    # summary(model, input_size=(3,416,415), batch_size=4)
    ...
    # Dataset change augment to false
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (imgs, targets, _, _) in enumerate(dataloader):
            ...
            t = time.time()
            # print(mloss)
            vis.plot_many_stack({'xy': mloss[0].item()})
            vis.plot_many_stack({'wh': mloss[1].item()})
            vis.plot_many_stack({'conf': mloss[2].item()})
            vis.plot_many_stack({'train loss': mloss[4].item()})
        ...
        vis.plot_many_stack({'test_loss': results[4]})
        # Update best loss
        test_loss = results[4]
        ...
        if save:
            ...
            if epoch > 0 and epoch % 10 == 0:
                torch.save(chkpt,
                           weights + '%s_%g.pt' % (os.path.basename(cfg).split('.')[0], epoch))
    ...
    # shutil.copy(cfg, weights + os.path.basename(cfg).split('.')[0]+".cfg")
    return results
